	...
	public function new(daStage:String)
	{
		...
		switch (daStage)
		{
			case 'flavioLand1':
				{
					camZoom = 0.9;
					var bg:FlxSprite = new FlxSprite(-100, 50).loadGraphic(Paths.image('stage/bg', 'flavio'));
					...
					swagBacks['bg'] = bg;
					toAdd.push(bg);
					var floor:FlxSprite = new FlxSprite(-300, 700).loadGraphic(Paths.image('stage/floor', 'flavio'));
					...
					toAdd.push(floor);
					var piano:FlxSprite = new FlxSprite(-200, 400).loadGraphic(Paths.image('stage/piano', 'flavio'));
					piano.antialiasing = FlxG.save.data.antialiasing;
					// Piano moves along the X axis for parallax but sticks with stage on Y axis.
					piano.scrollFactor.set(0.8, 0.9);
					piano.active = false;
					piano.scale.set(1.5, 1.5);
					swagBacks['piano'] = floor;
					toAdd.push(piano);
				}
			case 'flavioLand2':
				{
					camZoom = 0.9;
					var bg:FlxSprite = new FlxSprite(-100, 50).loadGraphic(Paths.image('stage/bg-phase2', 'flavio'));
					bg.antialiasing = FlxG.save.data.antialiasing;
					bg.scrollFactor.set(0.9, 0.9);
					bg.active = false;
					bg.scale.set(1.6, 1.6);
					swagBacks['bg'] = bg;
					toAdd.push(bg);
					var floor:FlxSprite = new FlxSprite(-300, 700).loadGraphic(Paths.image('stage/floor-phase2', 'flavio'));
					floor.antialiasing = FlxG.save.data.antialiasing;
					floor.scrollFactor.set(0.9, 0.9);
					floor.active = false;
					floor.scale.set(1.6, 1.6);
					swagBacks['floor'] = floor;
					toAdd.push(floor);
					var piano:FlxSprite = new FlxSprite(-200, 400).loadGraphic(Paths.image('stage/piano-phase2', 'flavio'));
					piano.antialiasing = FlxG.save.data.antialiasing;
					// Piano moves along the X axis for parallax but sticks with stage on Y axis.
					piano.scrollFactor.set(0.8, 0.9);
					piano.active = false;
				...
				case 'flavioLand3':
					{
						camZoom = 0.9;
						var bg:FlxSprite = new FlxSprite(-400, -475);
						bg.frames = Paths.getSparrowAtlas('stage/bg-phase3-sky','flavio');
						bg.animation.addByPrefix("Animation", "Animation", 9, true);
						bg.antialiasing = FlxG.save.data.antialiasing;
						bg.scrollFactor.set(0.9, 0.9);
						...
						// Necessary for anims?
						bg.active = true;
						bg.animation.play("Animation", true, false, 0);
						var floor:FlxSprite = new FlxSprite(-300, 600).loadGraphic(Paths.image('stage/floor-phase2', 'flavio'));
						floor.antialiasing = FlxG.save.data.antialiasing;
						floor.scrollFactor.set(0.9, 0.9);
						floor.active = false;
						floor.scale.set(1.6, 1.6);
						swagBacks['floor'] = floor;
						toAdd.push(floor);
						var wall:FlxSprite = new FlxSprite(-100, -200).loadGraphic(Paths.image('stage/bg-phase3-wall', 'flavio'));
						wall.antialiasing = FlxG.save.data.antialiasing;
						wall.scrollFactor.set(0.9, 0.9);
						wall.active = false;
						wall.scale.set(1.6, 1.6);
						swagBacks['wall'] = wall;
						toAdd.push(wall);
						var piano:FlxSprite = new FlxSprite(-200, 400).loadGraphic(Paths.image('stage/piano-phase2', 'flavio'));
						piano.antialiasing = FlxG.save.data.antialiasing;
